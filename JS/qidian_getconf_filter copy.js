...
(function () {
  ...
  // 书架顶部“每日导读/领福利-签到”模块兜底过滤（用于 bookshelf/refresh）
  const SHELF_BLOCK_RE = /(每日导读|签到|领福利)/;
  function shouldDropShelfCard(card) {
    if (!card || typeof card !== "object") return false;
    const keys = ["Title", "SubTitle", "Name", "Desc", "Description", "ButtonText", "RightText", "Text"];
    return keys.some((k) => typeof card[k] === "string" && SHELF_BLOCK_RE.test(card[k]));
  }

  function cleanBookshelfModules(node) {
    if (!node || typeof node !== "object") return;

    if (Array.isArray(node)) {
      for (let i = node.length - 1; i >= 0; i--) {
        const item = node[i];
        if (shouldDropShelfCard(item)) {
          node.splice(i, 1);
        } else {
          cleanBookshelfModules(item);
        }
      }
      return;
    }
    ...
    for (const [k, v] of Object.entries(node)) {
      if (Array.isArray(v)) {
        node[k] = v.filter((item) => !shouldDropShelfCard(item));
        node[k].forEach((item) => cleanBookshelfModules(item));
      } else if (v && typeof v === "object") {
        cleanBookshelfModules(v);
      }
    }
  }
  ...
  try {
    ...
    // ========== 书架刷新：兜底移除“每日导读/领福利-签到” ==========
    if (url.includes("argus/api/v2/bookshelf/refresh") || url.includes("argus/api/v1/bookshelf/refresh")) {
      if (body.Data && typeof body.Data === "object") {
        cleanBookshelfModules(body.Data);
      }
      return $done({ body: JSON.stringify(body) });
    }
