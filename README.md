...
- 菜鸟淘宝小程序模块已更新到 v8：全协议阻断两个广告专用传输主机；可见回退响应按广告 API 的数字位和结果数组清空，不依赖坑位、素材、文案或活动标识，并保留包裹和其它业务数据。
